...
@section('content')
  ...
  <div class="container items-center justify-center">
    ...
    <form class="m-auto flex flex-column shadow-sm col-10 p-6 bg-white rounded" method="POST" action="{{route('user.update',$user->id)}}">
      @csrf
      @method("PUT")
      <label class="py-2 font-bold text-base">Username:</label>
      <input class="block border rounded py-2 px-3 mb-2" type="text" name="name" placeholder="Name" value="{{$user->name}}" id="user-name"/>
      <label class="py-2 font-bold text-base" for="user-mail">Email:</label>
      <input class="block border rounded py-2 px-3 mb-2" type="email" name="email" placeholder="Email" value="{{$user->email}}" id="user-mail"/>
      ...
      <select class="block border rounded py-2 px-3 mb-2 text-base" name="region" id="user-region">
        ...
        @foreach ($regions as $region)
          <option value="{{$region->id}}" {{$user->region_id === $region->id ? "selected" : ''}}>{{ucfirst($region->name)}}</option>
        @endforeach
      </select>
    ...
    <div class="flex py-2">
      ...
      <a  href="{{route('user.index')}}"><button class="bg-gray-100 hover:bg-gray-300 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded"> Cancel</button> </a>
    </div>
  </form>
  </div>
@endsection