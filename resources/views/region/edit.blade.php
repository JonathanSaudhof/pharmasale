...
@section('content')
  ...
  <div class="container items-center justify-center">
    <div class="col-10 sticky flex justify-between items-center rounded mx-auto mb-6 px-6 py-3 shadow-sm bg-gray-50 border border-gray-500"><h1>🗺 Update region</h1>
    ...
    <form class="m-auto flex flex-column shadow-sm col-10 p-6 bg-white rounded" method="POST" action="{{route('region.update',$region->id)}}">
      @csrf
      @method("PUT")
      <label class="py-2 font-bold text-base">Region name:</label>
      <input class="block border rounded py-2 px-3 mb-2" type="text" name="name" value="{{ucfirst($region->name)}}" id="user-name"/>
    <div class="flex py-2">
